...
def main():
    ...
    parser.add_argument("--thickness", "-t", type=float, default=3.0,
                        help="Material thickness for STL generation (in mm)")
    ...
    # 支持 .ohd 文件直接加载
    if args.dxf_file.lower().endswith('.ohd'):
        design = OrigamiHandDesign.load(dxf_path)
        design.build_topology()  # 确保有 joints
    else:
